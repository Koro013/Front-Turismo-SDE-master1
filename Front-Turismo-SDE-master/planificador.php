...
if (!empty($_POST['destinos'])) {
    $ids = array_map('intval', $_POST['destinos']);
    $in = implode(',', array_fill(0, count($ids), '?'));
    $stmt = $pdo->prepare("SELECT * FROM destinos WHERE id IN ($in)");
    $stmt->execute($ids);
...
<body class="bg-body bebas-neue-regular">
  ...
  <main class="container pt-5">
    ...
    <form method="post">
      <section class="row justify-content-evenly ms-1 me-1">
        ...
        <div class="card btn btn-light col-lg-3 p-0 me-lg-3 mb-lg-3 mb-3 col-md-12 tarjeta-destinos">
          <img src="./img/<?php echo htmlspecialchars($d['imagen']); ?>" class="card-img-top tarjeta-imagen" alt="...">
          <div class="card-body">
            <p class="card-text fs-3">
              <input type="checkbox" name="destinos[]" value="<?php echo $d['id']; ?>"> <?php echo htmlspecialchars($d['nombre']); ?>
            </p>
          </div>
        </div>
        <?php endforeach; ?>
      ...
      <div class="mt-4">
        <label class="fs-4" for="mapSelect">Ver en mapa</label>
        <select id="mapSelect" class="form-select w-auto d-inline-block ms-2">
          <?php foreach ($destinos as $d): ?>
          <option data-lat="<?php echo $d['latitud']; ?>" data-lon="<?php echo $d['longitud']; ?>">
    ...
    <section class="text-center roboto-300 fs-3 mt-5">
      <p>Duración Total: <?php echo $totalDuracion; ?> min</p>
      <p>Costo Total: $<?php echo $totalCosto; ?></p>
      <a href="export_pdf.php?ids=<?php echo implode(',', $ids); ?>" target="_blank" class="btn btn-primary fs-4 ps-2 pt-1 pe-2 pb-1"><img class="img-pdf pe-1" src="./img/archivo-pdf.svg" alt="PDF">Exportar</a>
    </section>
    <?php endif; ?>
    <section class="col-12 my-5">
      <iframe id="mapFrame" class="container p-0" src="" style="border:0; height: 320px;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
    </section>
  </main>
